...
if runOptimization
    referenceDensities = 1000:50:1500;
    sizeReferenceDensities = size(referenceDensities);
    compensationDepths = [100e3:50e3:450e3, 469999];
        ...
        for depthIndex = 1:sizeCompensationDepths(2)
            ...
            if min(prattCrust, [], "all") > 910
                ...
                errors(depthIndex, densityIndex) = ...
                    rmse(modelDegreeVariance(SHbounds(1)+1:SHbounds(2)+1), ...
                    gravityDegreeVariance(SHbounds(1)+1:SHbounds(2)+1), "all");
            else
...
if runVisualization
    ...
    figure(1)
    colormap("turbo")
    imagesc(errors * 1e10, "AlphaData", heavierThanIce)
    bar = colorbar;
    bar.Label.String = "RMSE [mGal^2]";
